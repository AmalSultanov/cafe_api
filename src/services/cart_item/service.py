...
class CartItemService:
    ...
    async def _to_cart_item_read(self, item: CartItemModel) -> CartItemRead:
        meal = await self.meal_service.get_meal(item.meal_id)
        return CartItemRead(
            id=item.id,
    ...
    async def add_item_to_cart(
        ...
        if existing_item:
            ...
            upd_item = await self.repository.update_quantity(
                existing_item.id, new_quantity
            )
            return await self._to_cart_item_read(upd_item)

        meal = await self.meal_service.get_meal(item_data.meal_id)
        item_dict = item_data.model_dump()
        item_dict["cart_id"] = cart.id
        item_dict["unit_price"] = meal.unit_price

        item = await self.repository.create(item_dict)
        return await self._to_cart_item_read(item)
    ...
    async def get_cart_items(self, user_id: int) -> list[CartItemRead]:
        ...
        return [await self._to_cart_item_read(item) for item in items]
